...
class Firestore {
    ...
    suspend fun getCurrentRecipeId ():String {  //Christian
        ...
        if (responseList.size() > 1) {
            Log.d("DB-Call","overlapping date in DB")
            return responseList.toList()[0].id
        }
        else if (responseList.size() < 1) {
            Log.d("DB-Call","no recipe today<3, sorry " + responseList.toString())
            return ""
        }
        else {
    ...
    //funktionen var brugt til at uploade opskrifter til DB, da
